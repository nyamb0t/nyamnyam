...
async def number_delete(interaction: discord.Interaction, channel: discord.TextChannel = None):
    ...
    save_guild_data(guild_id, data)
    await interaction.response.send_message(f"{channel.mention} もう送らないからねーᴖ ̫ᴖ", ephemeral=True)

# --- Botにコマンド登録する setup 関数（__init__.py から呼び出す）
async def setup(bot: discord.Client):
    bot.tree.add_command(number_set)
    bot.tree.add_command(number_delete)
